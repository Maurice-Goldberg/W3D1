...
def substrings(string)
  new_array = []
  (0..string.length-1).each { |i|
    (0..string.length-1).each { |j|
      if i <= j
...
class Array
  def my_map(&prc)
    ...
    self.each { |ele| new_array << prc.call(ele) }
  ...
  def my_select(&prc)
    ...
    self.each { |ele| new_array << ele if prc.call(ele) } 
  ...
  def my_inject(&blk)
    if self[0].is_a?(String)
      storage_var = ""
    elsif self[0].is_a?(Integer)
      storage_var = 0
    end
    self.my_each { |ele|
      storage_var = blk.call(storage_var, ele)
    }
    storage_var
  end
end
...
def concatenate(strings)
  strings.my_inject { |sum_str, str| sum_str + str }
end